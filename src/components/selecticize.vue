<template>
  <div
    :class="[
      ...
      { focusedTarget: isActiveInput },
    ]"
    @click="handleFocus"
  >
    <div
      :class="['SelectTarget-chosen', `SelectTarget-target--${index}`]"
      @click="handleFocus"
    >
      <div
        :class="[
          'SelectTarget-selectedChoice',
          `SelectTarget-target--${index}`,
        ]"
    ...
    <ul
      :class="['SelectTarget-choices', `SelectTarget-target--${index}`]"
      ref="choicesref"
    >
      <li
        :class="[
...
export default {
  ...
  computed: {
    ...
    avaliableInputsSorted() {
      if (this.avaliableInputs.length > 0) {
        return this.avaliableInputs.sort((a, b) =>
    ...
    selectedInputsSorted() {
      if (this.selectedInputs.length > 0) {
        return this.selectedInputs.sort((a, b) =>
          a.title.localeCompare(b.title)
        );
      } 
      else 
    ...
    computedPlaceholder() {
      return this.selectedInputs.length <= 0 && !this.isActiveInput
        ? this.placeholder
        : "";
    },
  },
  ...
  mounted() {
    this.handleDocumentClick = (e) => {
      if (
        !e.target.classList.contains(`SelectTarget-target--${this.activeIndex}`)
      ) {
  ...
  methods: {
    ...
    handleInput() {
      ...
      const selectedInputIds = new Set(
        this.selectedInputs.map((item) => item.id)
      );
    ...
    removeChosenInput(input) {
      this.selectedInputs = this.selectedInputs.filter(
        (i) => i.id !== input.id
      );
